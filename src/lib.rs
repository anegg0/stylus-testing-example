...
mod tests {
    use super::*;
    use stylus_sdk::testing::{TestVMBuilder, Host};

    fn setup_test_host() -> impl Host {
        TestVMBuilder::new()
            .block_timestamp(100)
            .build()
    }
    ...
    fn test_give_cupcake() {
        ...
        let mut vending_machine = VendingMachine::from(host.clone());
    ...
    fn test_multiple_users() {
        ...
        // Verify balances
        assert_eq!(vending_machine.get_cupcake_balance_for(user1), U256::from(1));
        assert_eq!(vending_machine.get_cupcake_balance_for(user2), U256::from(1));

        // Advance time and give another cupcake to user1
        host.set_block_timestamp(106);
        assert!(vending_machine.give_cupcake_to(user1));

        // Verify final balances
        assert_eq!(vending_machine.get_cupcake_balance_for(user1), U256::from(2));
        assert_eq!(vending_machine.get_cupcake_balance_for(user2), U256::from(1));
    }
}
